/**
 * SQLite schema. MUST stay column-for-column identical (names, nullability,
 * uniques) with ./pg.ts — enforced by test/schema-parity.test.ts.
 */
import {
  index,
  integer,
...
export const orgConnectors = sqliteTable("org_connectors", {
  id: id(),
  orgId: text("org_id")
    .notNull()
    .references(() => orgs.id, { onDelete: "cascade" }),
...
export const translationKeys = sqliteTable(
  "translation_keys",
  {
    id: id(),
    projectId: text("project_id")
...
export const keyAliases = sqliteTable(
  "key_aliases",
  {
    id: id(),
    projectId: text("project_id")
...
export const importEntries = sqliteTable(
  "import_entries",
  {
    id: id(),
    jobId: text("job_id")
